...
- [`vm-spec.md`](vm-spec.md) — how `ApcOptimizer/VmSpec/` is built: the VM-level statement,
  `StepLayout`/`Circuit.legalGuest`, the soundness connection and why it doesn't give legality or
  completeness for free, what's proven against a real APC, and the current known limitations.
